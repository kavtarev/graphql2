import { Slide } from './infrastructure/models/Slide'
...
import { ServerConfig, ConfigFactory, LogConfig, DbConfig, ConfigFileChain } from '@melmedia/config'
...
export class AppModule extends Module {
    ...
    public async initDiContainer(container: Container, allApplicationModules: Module[]) {
        ...
        container.bind<Logger>(Type.AppLogger).toConstantValue(loggerFactory.create('app'))
        container.bind<Logger>(Type.DbLogger).toConstantValue(loggerFactory.create('db'))
        container.bind<Logger>(Type.AccessLogger).toConstantValue(loggerFactory.create('access'))

        container
            .bind<Connection>(Type.DbConnection)
            .toConstantValue(await new DbConnectionFactory().create(allApplicationModules))

        container
            .bind<Repository<models.Slide>>(Type.SlideDataRepository)
            .toConstantValue(getRepository(models.Slide))
    }
